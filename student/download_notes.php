...
Edit Profile
           ...
           <div class="md:w-3/4 md:relative md:left-1/4 md:px-8 pt-8 main-content">
            <div class="top-bar fixed top-0 bg-teal-600 left-0 max-w-screen w-full flex px-2 items-center justify-between text-white py-2 text-lg">
                <h2 onclick="showMenu()" class="font-bold gap-1 flex items-center"><span class="ri-menu-2-line md:hidden text-2xl"></span>Notes and Resources</h2>
  ...
  slt.innerText = classes.length
     ...
     <h2 class="my-10 relative -z-20 text-2xl pl-20 py-10 font-bold"><img src="../global/dd10.png" class="-top-1 md:w-2/12 md:top-3 -left-8 w-2/5 object-cover absolute object-center z-10">Download Notes for <?php echo htmlspecialchars($class_id); ?></h2>

    <?php if (empty($notes)): ?>
<i class="ri-cloud-off-fill text-9xl block text-center mt-20 text-gray-700 "></i>
        <p class="text-center text-teal-700 mb-20">No available office hours at the moment.</p>    <?php else: ?>
        <main>
        <?php foreach ($notes as $note): ?>
            <div class="border border-gray-300 rounded mx-1 my-2 p-2">
                <strong class="block text-2xl py-2"><?php echo htmlspecialchars($note['title']); ?></strong>
                <p class="ml-3 text-gray-900 mb-3">
                <?php echo $note['description']; ?></p>
               <p class="text-sm text-teal-600"><?php echo hrd($note['created_at']); ?></p>
                 <a class="bg-teal-600 text-white py-1 px-3 rounded"href="../teacher/<?php echo $note['path']; ?>" download>Download</a>
            </div>
        <?php endforeach; ?>
        </main>
    <?php endif; ?>
