      ...
      setTimeout(()=>{
        ...
        <div class="md:p-3 md:w-3/4 md:relative md:left-1/4 p-3 mt-12 main-content">
            <div class="top-bar fixed top-0 bg-teal-600 left-0 w-full flex px-2 items-center justify-between text-white py-2 text-lg">
                <h2 onclick="showMenu()" class="font-bold gap-1 flex items-center"><span class="ri-menu-2-line text-2xl md:hidden"></span>Manage Students</h2>
...
<h2 class="relative mt-9 -z-10 text-2xl pl-20 py-10 font-bold"><img src="../global/dd2.png" class="-top-1 -left-8 w-2/5 md:w-2/12 md:top-3 object-cover absolute object-center -z-20">Students Registered Under Your School</h2>

<form method="GET" class="grid my-2 md:gap-2 gap-0.5 grid-cols-12 grid-rows-1 w-full">
    ...
    <button type="submit" class="col-span-1 bg-teal-600 text-white aspect-square md:aspect-video rounded ri-search-2-line"></button>
</form>
...
<table class="w-full border border-collapse">
    ...
    <?php while ($row = $result->fetchArray(SQLITE3_ASSOC)): ?>
        <tr>
            ...
            <td>
                <a href="edit_student.php?id=<?php echo $row['id']; ?>" class="bg-green-600 px-2 block text-sm py-1 m-0.5 rounded text-center text-white">Edit</a>
                <a href="delete_student.php?id=<?php echo $row['id']; ?>" class="button delete bg-red-700 text-sm px-2 py-1 mx-0.5 text-center box-border block rounded text-white" onclick="return confirm('Delete this student?');">Delete</a>
            </td>
        </tr>
    <?php endwhile; ?>
</table>
