...
if (!isset($_SESSION['school_logged_in']) || $_SESSION['school_logged_in'] !== true) {
    die("Access denied. Please log in as a school.<a href='login.php'>Log in</a>");
}
...
<body>
  <h2 class="top"><span onclick="history.back()" class="ri-arrow-left-s-line"></span>Timetable<span class="ri-printer-fill"> Print</span></h2>
  <select id="classes" onchange="populate(tt,this.value)"></select>
  <button id="edit-btn"  onclick="toggleEditMode()">Edit this Timetable</button>
  <button id="edit-btn"  onclick="document.querySelector('.modal').style.display= 'grid'">Add a New Class Timetable</button>
  <button id="save-btn" onclick="saveChanges()" style="display: none;background:rgb(20,184,166)" style="display: none;background:rgb(20,184,166)">Save</button>
      <div id="con">
  ...
  <script>
    ...
    Object.keys(tt).map(key => {
      cs.innerHTML += `<option value="${key}">${key}</option>`;
    });
        ...
        if (key == "periods") {
          ...
          for (i = 0; i < data[key].length; i++) {
            if (editMode) {
              ...
              <option ${data[key][i][2]=='id'?'selected':''} value="id">Registered Teachers</option>
