...
$class_list = $classes->fetchArray(SQLITE3_ASSOC);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($title && $class_id && isset($_FILES['note_file'])) {
        $file = $_FILES['note_file'];
        $ext = pathinfo($file['name'], PATHINFO_EXTENSION);
        $filename = 'notes/' . uniqid() . '.' . $ext;
        move_uploaded_file($file['tmp_name'], $filename);

        // Insert into notes table
        $conn->exec("INSERT INTO notes (class_id, teacher_id, title, description,path)  VALUES ('$class_id', $teacher_id, '$title', '$description','$filename')");

        $note_id = $conn->lastInsertRowID();
        ...
        activate();
           ...
           <div class="p-4 md:w-3/4 md:relative md:left-1/4 md:px-8 main-content">
            <div class="top-bar fixed top-0 bg-teal-600 left-0 max-w-screen w-full flex px-2 items-center justify-between text-white py-2 text-lg">
                <h2 onclick="showMenu()" class="font-bold gap-1 flex items-center"><span class="ri-menu-2-line text-2xl md:hidden"></span>Notes</h2>
...
<h2 class="my-10 relative -z-20 text-2xl pl-20 py-10 font-bold"><img src="../global/dd6.png" class="-top-1 -left-8 w-2/5 md:top-3 md:w-2/12 object-cover absolute object-center z-10">Upload New Notes</h2>
<form method="post" class="md:px-16 mt-12" enctype="multipart/form-data">
...
<?php if (!empty($error)) echo "<p class='bg-red-100 border border-red-500 p-2  rounded text-red-500'>$error</p>"; ?>
    ...
    <label class="text-gray-400 uppercase text-sm">Description:</label><br>
    <textarea class="border border-teal-500  p-2 outline-teal-500 mb-5 w-full rounded " name="description" rows="4" ></textarea><br><br>
    
    <label class="text-gray-400 uppercase text-sm">Select Class:</label>
    <select class="border border-teal-500  p-2 outline-teal-500 mb-5 w-full rounded " name="class_id" id="cl" required>
        <option value="">-- Select --</option>
    </select><br><br>

    <label class="text-gray-400 uppercase text-sm">Choose File:</label> <input type="file" name="note_file" class="border border-teal-500  p-2 outline-teal-500 mb-5 w-full rounded " required><br><br>

    <button class="bg-teal-600 text-white py-1.5 px-2.5 rounded"  type="submit">Upload</button>
</form>
<p class="block underline text-center text-teal-600 text-sm"><a href="dashboard.php">Back to Dashboard</a></p>
<script>
  var classes = [<?php echo $class_list;?>];
